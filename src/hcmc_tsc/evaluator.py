...
def evaluate_scenario(config: EvalConfig, record: dict[str, Any]) -> dict[str, Any]:
...
def evaluate(config: EvalConfig) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if config.gui and config.jobs > 1:
        raise ValueError("--gui cannot be used with --jobs > 1.")
    output = Path(config.output_dir)
    ...
    if config.jobs == 1:
        rows = [evaluate_scenario(config, record) for record in tqdm(scenarios, desc=f"evaluate {config.method}")]
    else:
        rows = []
        with ProcessPoolExecutor(max_workers=config.jobs) as executor:
            futures = [executor.submit(evaluate_scenario, config, record) for record in scenarios]
            for future in tqdm(futures, desc=f"evaluate {config.method}", total=len(futures)):
